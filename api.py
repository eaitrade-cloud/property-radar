...
def find_best_benchmarks(
    ...
    return [
        item
        for item in matches
        if item["match_score"] == best_score
    ]
...
def calculate_weighted_benchmark(
    ...
    for item in matches:
        ...
        used.append({
            "city": benchmark.get("city"),
            "micro_location": benchmark.get(
                "micro_location"
            ),
            "property_type": benchmark.get(
                "property_type"
            ),
            "benchmark_mad_m2": value,
...
def distance_adjustment(distance_km):
...
def size_adjustment(built_area):
    area = safe_float(
        built_area
    )
...
def analyse_market(
    ...
    if not built_area:
        result["reason"] = "Built area required"
        return result
    ...
    result.update({
        ...
        "raw_benchmark_mad_m2": raw_benchmark,
        "adjusted_benchmark_mad_m2": adjusted_benchmark,
        "benchmark_mad_m2": adjusted_benchmark,
        "difference_percent": difference,
        "market_position": market_position,
        "sample_size": sample_size,
        "distance_km": resolved_distance,
        "distance_factor": distance_factor,
        "size_factor": area_factor,
